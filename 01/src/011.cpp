...
int main (int argc, char *argv[])
{
    ...
    auto db_move = [&rnd](point<double,1>& p){p[0]=rnd.Rannyu(); return (bool)1;};
    ...
    array<string,2> path = {"01/out/011-mean_value.csv","01/out/011-variance.csv"};
    for(int i=0;i<2;++i) {
    ...
    for (int i=0; i<N_TESTS; ++i) {
        ...
        for (int t : hyst) chi2+=(t-expected_value)*(t-expected_value);
