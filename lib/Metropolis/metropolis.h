...
/* Useful if ratio of pdf can be optimized */
template<int DIM> 
bool Mrt2UnifOpt(point<double,DIM>& p, function<double(point<double,DIM>)> pdf_ratio, Random& rnd, double delta){
    auto m = [&rnd,&delta](point<double,DIM>& p){for(double& x : p.ToArray())x+=rnd.Rannyu(-delta,delta); return 1;};
    point<double,DIM> q(p.ToArray(),m);
    q.Move();
    double test = pdf_ratio(q);
	if (test >= 1.){
        p.ToArray()=q.ToArray();
        return 1;
    } else if(rnd.Rannyu() < test){
...
template<int DIM> 
bool Mrt2GaussOpt(point<double,DIM>& p, function<double(point<double,DIM>)> pdf_ratio, Random& rnd, double sigma){
    auto m = [&rnd,&sigma](point<double,DIM>& p){for(double& x : p.ToArray())x+=rnd.Gauss(0,sigma); return 1;};
    point<double,DIM> q(p.ToArray(),m);
    q.Move();
    double test = pdf_ratio(q);
