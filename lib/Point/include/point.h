...
template <typename T, int N> point<T, N> ToPoint(T x1, ...);
...
template <typename T, int N> void point<T, N>::Jump(array<T, N> p) { p_ = p; }
...
template <typename T, int N> double point<T, N>::Lenght2() {
  double l = 0;
  for (T x : p_)
    l += x * x;
  return l;
