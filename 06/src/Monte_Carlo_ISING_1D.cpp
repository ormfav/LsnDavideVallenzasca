  ...
  for (int i = 0; i < nspin; ++i) {
    ...
    if (metro == 1) // Metropolis
    {
      ...
      if (deltaE <= 0) {
        s[o] = -s[o];
        accepted++;
      } else if (rnd.Rannyu() < exp(-beta * deltaE)) {
        s[o] = -s[o];
        accepted++;
      }
    } else // Gibbs sampling
...
void Measure() {
  ...
  walker[ix] = m * m; // only valid for h = 0
}
...
void Averages(int iblk) // Print results for current block
{
  ...
  foutE << setw(wd) << iblk << setw(wd) << stima_u << setw(wd)
  ...
  foutC << setw(wd) << iblk << setw(wd) << stima_c << setw(wd)
  ...
  foutM << setw(wd) << iblk << setw(wd) << stima_m << setw(wd)
  ...
  foutX << setw(wd) << iblk << setw(wd) << stima_x << setw(wd)
        << glob_av[ix] / (double)iblk << setw(wd) << err_x << endl;
  foutX.close();
