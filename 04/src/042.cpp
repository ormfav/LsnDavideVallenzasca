...
void Input(void) {
  ...
  ReadInput.open(indir + "input." + state);

  ReadInput >> iNVET; // 0=MD; 1=MC
  ReadInput >> restart;

  if (restart)
    Seed.open(savedir + state + "/seed.out");
  else {
    Seed.open(indir + "seed.in");
    // Clear output files
    cout << "Clearing files in " << outdir + state << endl << endl;
    ofstream clear;
    clear.open(outdir + state + "/output_epot.dat", ios::trunc);
    clear.close();
    clear.open(outdir + state + "/output_ekin.dat", ios::trunc);
    clear.close();
    clear.open(outdir + state + "/output_temp.dat", ios::trunc);
    clear.close();
    clear.open(outdir + state + "/output_etot.dat", ios::trunc);
    clear.close();
    clear.open(outdir + state + "/output_pres.dat", ios::trunc);
    clear.close();
  }
  ...
  if (restart) {
    ReadConf.open(savedir + state + "/config.out");
    ReadVelocity.open(savedir + state + "/velocity.out");
    for (int i = 0; i < npart; ++i)
      ReadVelocity >> vx[i] >> vy[i] >> vz[i];
  } else {
    ReadConf.open(indir + "config.in");
...
void ConfFinal(void) {
  ...
  WriteConf.open(savedir + state + "/config.out");
  WriteVelocity.open(savedir + state + "/velocity.out");
  for (int i = 0; i < npart; ++i) {
  ...
  string tempstr = savedir + state + "/seed.out";
