...
#include "../in/021-conf.inl"
  ...
  array<string, 2> path = {"02/out/021-uniform.csv",
                           "02/out/021-importance_o1.csv"};
  for (int i = 0; i < 2; ++i) {
    fout[i].open(path[i]);
    if (!fout[i].is_open()) {
  ...
  //     return x;
  // };
  // dataBlocks int_importance_oN(N_BLOCKS, STEPS_PER_BLOCK,
  // bind(f_importance_oN,3));
